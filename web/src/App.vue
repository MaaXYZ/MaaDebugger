...
const selectTheme = {
  trailingIcon:
    "transition-transform ease-in-out duration-200 group-data-[state=open]:rotate-180",
};
...
onMounted(async () => {
  await debugWorkspaceSettingsStore.syncPipelineChecker();
  ...
  wsClient.connect({
    ...
    onScreenshotFrame(data) {
      if (
        screenshotPaused.value ||
        screenshotOverlayState.value === "disconnected" ||
    ...
    onWatchResourceChanged(path) {
      ...
      signalStore.setChangedPath(path);
    },
