<template>
    <div class="rounded-lg border border-default transition-colors hover:bg-elevated">
        <!-- Header: always visible -->
        <div class="flex flex-row items-center gap-2 min-w-0 p-3 cursor-pointer select-none"
            ...
            <span v-if="!expanded && node.reco.length > 0"
                  class="text-xs text-dimmed tabular-nums ml-auto shrink-0">
                ...
                <div class="flex flex-col gap-2 px-3 pb-3">
                    ...
                    <div v-if="node.reco.length > 0" class="flex flex-col gap-1.5">
                        ...
                        <div class="pl-5 flex flex-col gap-1.5">
                            <UButton v-if="hasHiddenRounds" size="xs" variant="ghost" color="neutral"
                                     class="self-start" @click.stop="showAllRounds = !showAllRounds">
                            ...
                            <template v-for="(nextList, idx) in visibleReco" :key="recoOffset + idx">
                                <NextListItem :next-list="nextList"
