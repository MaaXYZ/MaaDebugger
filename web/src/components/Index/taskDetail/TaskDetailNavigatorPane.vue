<template>
    <div class="flex h-full flex-col gap-3 rounded-xl border border-default bg-default/40 p-3">
        ...
        <UEmpty v-if="tasks.length === 0" icon="i-lucide:list-checks" title="No Tasks" description="Run a task to start inspecting details." />
            ...
            <div class="flex flex-col gap-2">
                ...
                <div class="flex flex-col gap-2">
                    ...
                    >
                        ...
                        <UBadge :color="task.status === 'success' ? 'success' : task.status === 'failed' ? 'error' : 'info'" variant="subtle" size="xs" class="capitalize shrink-0">
                            {{ task.status }}
                        </UBadge>
                    </button>
                </div>
            ...
            <div v-if="activeTask" class="flex min-h-0 flex-1 flex-col gap-2">
                ...
                <div class="min-h-0 flex-1 overflow-y-auto pr-1">
                    <div class="flex flex-col gap-2">
                        ...
                        >
                            <UIcon
                                :name="node.msg.node_id === entryNodeId ? 'i-lucide-flag' : 'i-lucide-workflow'"
